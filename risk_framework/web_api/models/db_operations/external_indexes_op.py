...
from risk_framework.species_models.ch_model import CHModel
from risk_framework.species_models.pa_model import PAModel
...
def retrieve_or_calculate_pa(country_code, wkt_polygon, geo_id, db):
    ...
    query = query.filter(
        ProtectedAreaIndexDB.geo_id == geo_id,
    )

    existing_record = query.first()
    # If record exists, retrieve it and return cached result
    if not existing_record:
        existing_record = run_and_create_new_pa_record(
            geo_id,
            country_code.upper(),
            wkt_polygon,
            db
...
def run_and_create_new_pa_record(geo_id, country_code, wkt_polygon, db):
    ...
    model = PAModel(country_code, wkt_polygon=wkt_polygon, db=db)

    result = model.run()

    scenario_record = create_pa_and_raster_records(
        geo_id, result, db
    )
    return scenario_record


def create_pa_and_raster_records(geo_id, result, db):
    country_code = result['country_code']
    wkt_polygon = result['wkt_polygon']
    ...
    new_record = ProtectedAreaIndexDB(
        id=str(uuid.uuid4()),
        geo_id=geo_id,
        value_raster_id=new_raster_data.id,
        geometry=wkt_polygon,
