...
def relocate_with_linger_and_purge(aoi_prefix, csv_prefix, dst_dir,
                                   required_exts=(".shp",".shx",".dbf",".prj"),
                                   optional_exts=(".cpg",".fix",".zip"),
                                   timeout_sec=600, poll_sec=5):
    os.makedirs(dst_dir, exist_ok=True)
    SEARCH_ROOTS = [
        os.path.join(ALL_SPECIES_ROOT),
    ]
    CSV_PATTERNS = [f"{csv_prefix}.csv", f"{csv_prefix}.CSV",
                    f"{csv_prefix}*.csv", f"{csv_prefix}*.CSV"]
    AOI_PATTERNS = [f"{aoi_prefix}{ext}" for ext in (*required_exts, *optional_exts)]

    def sweep_once(move_only=True):
        moved_any = False
        for root in SEARCH_ROOTS:
            for pat in CSV_PATTERNS:
                for src in glob.glob(os.path.join(root, "**", pat), recursive=True):
                    if os.path.commonpath([dst_dir]) == os.path.commonpath([dst_dir, src]):
                        continue
                    dst = os.path.join(dst_dir, os.path.basename(src))
                    ok = _force_move(src, dst) if move_only else (os.path.exists(dst) and os.remove(src) is None)
                    moved_any = moved_any or bool(ok)
        for root in SEARCH_ROOTS:
            for pat in AOI_PATTERNS:
                for src in glob.glob(os.path.join(root, "**", pat), recursive=True):
                    if os.path.commonpath([dst_dir]) == os.path.commonpath([dst_dir, src]):
                        continue
                    dst = os.path.join(dst_dir, os.path.basename(src))
                    ...
                    moved_any = moved_any or bool(ok)
        # unzip if needed
        zip_path = os.path.join(dst_dir, f"{aoi_prefix}.zip")
        if os.path.exists(zip_path):
            try:
                ...
                os.remove(zip_path)
                moved_any = True
                print("Unzipped + removed:", zip_path)
            except Exception as e:
                print("[Warn] unzip failed:", e)
        return moved_any

    def have_required():
        csv_ok = any(os.path.exists(os.path.join(dst_dir, f"{csv_prefix}{ext}")) for ext in (".csv",".CSV"))
        aoi_ok = all(os.path.exists(os.path.join(dst_dir, f"{aoi_prefix}{ext}")) for ext in required_exts)
        return csv_ok and aoi_ok

    end = time.time() + timeout_sec
    last_ok = False
    while time.time() < end:
        _ = sweep_once(move_only=True)
        ok = have_required()
        if ok and last_ok:
            print("All required files are present in", dst_dir)
            break
        last_ok = ok
        time.sleep(poll_sec)
    print("Final purge of leftovers in export roots (if any)...")
    _ = sweep_once(move_only=False)
...
def remove_duplicates(fc, grain):
...
def load_gbif_data(species_name, country_code, exclude_year=None, exclude_months=None):
    ...
    """
    df_gbif = get_gbif_species_data(species_name, country_code)
    if df_gbif.empty:
...
def prepare_predictors(aoi_ee, grain_size):
...
def filter_variables_by_vif(df_, threshold=10):
...
def generate_presence_absence_data(data_fc, predictors, bands, grain_size):
...
def export_to_drive(aoi_ee, samples_table, species_safe, ee_export_folder, aoi_prefix, csv_prefix):
...
def export_to_local(aoi_ee, samples_table, species_safe, data_dir, aoi_prefix, csv_prefix):
...
def export_aoi_csv_files(aoi_ee, samples_table, species_safe, data_dir, aoi_prefix, csv_prefix, export_to_google_drive=False):
    ...
    if export_to_google_drive:
        ...
        # Local export
        res = export_to_local(aoi_ee, samples_table, species_safe, data_dir, aoi_prefix, csv_prefix)

        print("✅ Export completed successfully!")
        return res

def verify_files_exist(aoi_prefix, csv_prefix, dst_dir):
...
def find_aoi_shapefile(data_dir, spec_safe):
...
def load_aoi_or_fallback(data_dir, spec_safe):
    aoi_shp = find_aoi_shapefile(data_dir, spec_safe)
    aoi = None
    if aoi_shp:
...
def export_and_interpolate_predictors(aoi_bbox, aoi=None):
...
def interpolate_raster_nearest(in_tif, out_tif):
...
def interpolate_raster_nearest_fast(in_tif, out_tif):
    with rio.open(in_tif) as src:
        meta = src.meta.copy()
        nd = src.nodata
        out = []
...
def interpolate_files_or_skip():
    ...
    if os.path.exists(WC_CURRENT_DRIVE) and not os.path.exists(WC_CURRENT_INTERP):
        interpolate_raster_nearest(WC_CURRENT_DRIVE, WC_CURRENT_INTERP)
    ...
    if os.path.exists(ENV_1KM_DRIVE) and not os.path.exists(ENV_1KM_INTERP):
        interpolate_raster_nearest(ENV_1KM_DRIVE, ENV_1KM_INTERP)
...
# ============================================================
def detect_bands_from_csv(csv_path):
...
def return_and_save_bands_list():
    bands = detect_bands_from_csv(TRAIN_CSV)
    if not bands:
...
def read_env_band(src_env, name, H, W):
    name_l = name.lower()
...
def build_stack_dynamic(band_list, out_path, clim_path, env_path):
    ...
    with rio.open(env_path) as src_env:
        if any(b.lower() == 'aspect' for b in band_list):
            elev = read_env_band(src_env, 'elevation', H, W)
            env_cache['elevation'] = elev
            env_cache['aspect'] = compute_aspect_from_elev(elev, transform)
        for b in band_list:
            bl = b.lower()
            if bl in {'elevation','slope','hillshade','tcc'} and bl not in env_cache:
                env_cache[bl] = read_env_band(src_env, bl, H, W)
    clim_cache = {}
    with rio.open(clim_path) as src_clim:
        for b in band_list:
            if re.match(r'(?i)^bio\d{2}$', b) and b not in clim_cache:
                clim_cache[b] = read_clim_band(src_clim, b)
    ...
    for b in band_list:
        ...
        arrays.append(soft_fill(arr)); names.append(b)
...
def build_stack_if_existing_current_interp(bands):
    if os.path.exists(WC_CURRENT_INTERP) and os.path.exists(ENV_1KM_INTERP):
        _ok = build_stack_dynamic(bands, CURRENT_STACK, WC_CURRENT_INTERP, ENV_1KM_INTERP)
    else:
        print("⏭️ Skipped building CURRENT stack (inputs missing).")

def save_current_model_summary(train_bands, auc_mean, thr_best, data_dir, species_safe, scenario_model):
...
def project_current_maps(pipe, thr_best, current_stack_path, current_prob_path, current_bin_path, nodata_val):
...
def train_current_rf_model(train_csv_path, current_stack_path, model_bundle_path,
    ...
    save_current_model_summary(train_bands, auc_mean, thr_best, data_dir, species_safe, scenario_model)
    project_current_maps(pipe, thr_best, current_stack_path, current_prob_path, current_bin_path, nodata_val)



def train_multi_models(train_csv_path, current_stack_path, data_dir, species_safe, scenario_model):
...
def find_future_tif(model, ssp, period, roots):
...
def _predict_proba_safe(model, Xdf):
...
def load_models_if_needed(data_dir, species_safe):
    ...
    try:
        ...
        print("ℹ️ Loaded models from data folder.")
        return models, best_xgb, best_lgb, best_cb, ev, train_bands_final
    except Exception as e:
        print("⚠️ Could not load saved models:", e)
        return None, None, None, None, None, None

def process_future_maps(scenarios, periods, scenario_model, future_tif_roots, aoi_gdf_wgs84,
    ...
    if models is None:
        models, best_xgb, best_lgb, best_cb, ev, train_bands_final = load_models_if_needed(data_dir, species_safe)
    ...
    for ssp in scenarios:
        for period in periods:
            future_tif_global = find_future_tif(scenario_model, ssp, period, future_tif_roots)
            if future_tif_global is None:
                print(f"❗ Future global tif not found for {ssp} {period} in any root.")
                continue

            future_stack = prepare_future_stack(
...
def prepare_future_stack(future_tif_global, aoi_gdf_wgs84, input_dir, scenario_model, ssp, period,
                        env_1km_interp, bands, nodata_val):
    ...
    interpolate_raster_nearest_fast(fut_clipped, fut_interp)
    ...
    if os.path.exists(fut_interp) and os.path.exists(env_1km_interp):
        ok = build_stack_dynamic(bands, future_stack, fut_interp, env_1km_interp)
        if not ok:
...
def export_future_maps(future_stack_path, train_bands_final, models_dict, species_dir,
                      scenario_model, ssp, period, nodata_val, default_bin_thr):
    ...
    for name, mdl in models_dict.items():
        ...
        if np.any(valid_mask):
            with _silence_out, _silence_err:
                probs_all[valid_mask] = _predict_proba_safe(mdl, flat_df.iloc[valid_mask])
...
def clip_future_raster(future_tif_global, input_dir, gcm_model, scenario, period, aoi_shp):
...
def clipand_interpolate_future_raster_for_all(data_dir, input_dir, scenarios, periods, scenario_model, future_tif_roots, spec_safe):
    aoi_shp = find_aoi_shapefile(data_dir, spec_safe)
    for ssp in scenarios:
        for period in periods:
            future_tif_global = find_future_tif(scenario_model, ssp, period, future_tif_roots)
            ...
            fut_clipped, fut_interp, fut_stack = clip_future_raster(future_tif_global, input_dir, scenario_model, ssp, period, aoi_shp)
...
def load_models(data_dir, species_safe):
...
def prepare_future_stack_data(future_stack_path, train_bands_final, nodata_val):
...
def export_meta_and_raster_to_json_dict(raster_data, meta):
    meta_copy = meta.copy()
...
def export_meta_and_raster_to_json(raster_data, meta, output_path):
    """Export raster data and metadata as JSON file alongside the GeoTIFF."""

    # Create JSON data structure
    json_data = export_meta_and_raster_to_json_dict(raster_data, meta)
...
def export_models_predictions(models, flat_df, valid_mask, H, W, meta,
    ...
    for name, mdl in models.items():
        ...
        with rio.open(out_prob, "w", **prob_meta) as dst:
            dst.write(prob_raster, 1)
        prob_json_output = export_meta_and_raster_to_json(prob_raster, prob_meta, out_prob)
        print("✔ Probability:", out_prob)
        ...
        export_meta_and_raster_to_json(bin_raster, bin_meta, out_bin)
        print("✔ Binary     :", out_bin)


def run_model_prob_prediction(model_name, model, flat_df, valid_mask, H, W, meta):
    ...
    prob_meta = meta.copy()
    prob_meta.update(count=1, dtype="float32", nodata=np.nan, compress="DEFLATE", predictor=3)
    prob_json_output = export_meta_and_raster_to_json_dict(prob_raster, prob_meta)
    return prob_json_output



def predict_and_export_future_maps(data_dir, species_safe, future_stack_path,
                                 fut_prob_dir, fut_bin_dir, nodata_val, default_bin_thr):
    """Main function to load models and export future predictions."""
    # Load models
    models, train_bands_final = load_models(data_dir, species_safe)


    # Prepare future stack data
    meta, H, W, flat_df, valid_mask = prepare_future_stack_data(
        future_stack_path, train_bands_final, nodata_val
    )

    # Export predictions
    prob_json_output = export_models_predictions(
...
def predict_and_export_future_maps_for_all(data_dir, input_dir, species_dir, scenario_model, scenarios, periods, species_safe, default_bin_thr):
    for ssp in scenarios:
        for period in periods:
            fut_parent   = os.path.join(species_dir, scenario_model, ssp, period)
            fut_prob_dir = os.path.join(fut_parent, "probability")
            ...
            predict_and_export_future_maps(
                data_dir=data_dir,
...
def predict_future_species_suitability(data_dir, input_dir, scenarios, periods, species_safe, summary_only=False):
    models, train_bands_final = load_models(data_dir, species_safe)
    model_name = 'Ensemble'
    ...
    for ssp in scenarios:
        ...
        for period in periods:
            ...
            prob_json_output = run_model_prob_prediction(
